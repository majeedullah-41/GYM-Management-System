...
function LoginForm({ onComplete }: { onComplete: (user: AuthUser) => void }) {
  ...
  return <AuthFrame><h2 className="mb-1 text-lg font-semibold text-text-primary">Login</h2><p className="mb-5 text-sm text-text-muted">Sign in to continue.</p><form className="space-y-3" onSubmit={(e) => { e.preventDefault(); run(async () => onComplete(await login({ username, password }))); }}><Input label="Username" required autoFocus autoComplete="username" value={username} onChange={(e) => setUsername(e.target.value)} /><PasswordInput showLabel label="Password" required autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} />{error && <p className="text-sm text-danger">{error}</p>}<Button type="submit" loading={busy} className="w-full">Login</Button><button type="button" className="w-full text-sm font-medium text-primary hover:underline" onClick={() => run(async () => { const result = await getRecoveryQuestion(username); if (!result.recovery_available || !result.security_question) { setStep("unavailable"); return; } setQuestion(result.security_question); setStep("answer"); })}>Forgot Password?</button></form></AuthFrame>;
}
